...
def preprocess_user_input(user_input):
    ...
    word_token_1 = nltk.word_tokenize(user_input)
    
    # convert the user input into its root words : stemming
    word_token_2 = get_stem_words(word_token_1, ignore_words) 
    
    # Remove duplicacy and sort the user_input
    word_token_2 = sorted(list(set(word_token_2)))

    # Input data encoding : Create BOW for user_input
    for word in words:            
        if word in word_token_2:              
